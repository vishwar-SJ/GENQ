...
const RISK_COLORS = {
  ...
  'No Risk': { bg: '#dcfce7', text: '#166534', dot: '#22c55e' },
  'Average Risk (Carrier possible)': { bg: '#fef3c7', text: '#92400e', dot: '#f59e0b' },
  'High Risk': { bg: '#fecaca', text: '#991b1b', dot: '#ef4444' },
...
export default function OffspringRiskCard({ result, index }) {
  const colors = RISK_COLORS[result.risk_label] || RISK_COLORS['Insufficient data'];
  const isPolygenic = result.is_polygenic === 'true';
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
              {isPolygenic ? 'Polygenic Expected Risk' : 'Mendelian Inheritance'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-auto pt-2 border-t" style={{ borderColor: 'var(--color-border-light)' }}>
        <p className="text-sm font-semibold" style={{ color: 'var(--color-text-secondary)' }}>
          {result.probability_text}
        </p>
        {isPolygenic && result.percentile != null && (
          <p className="text-xs mt-1" style={{ color: 'var(--color-text-muted)' }}>
            Expected Percentile: <strong style={{ color: 'var(--color-text-secondary)' }}>{result.percentile}%</strong>
          </p>
        )}
      </div>
    </div>
